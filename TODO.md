...
- [x] 連鎖完了を検知する仕組みを追加
- [x] 連鎖中は `droppedPositions` と `rngState` を一時的に保持
- [x] 連鎖完了後（または連鎖なしの場合は即座に）スナップショットを作成
- [x] ゲーム開始時に初期状態の snapshot[0] を作成

### 2. Resume / Fork / Shuffle の修正

- [x] `applyGravity` の呼び出しを削除（不要になる）
- [x] 連鎖完了後の状態から開始することを確認

### 3. Replay 画面の修正

- [x] 空フィールドからスタートするように変更
- [x] Next ボタンの動作を変更:
...
- [x] 既存の operationHistory を新形式に変換するか、互換性を保つか検討
  - 既存データはクリア済みのため、互換性対応は不要
...
2. **状態監視方式**: `phase` が `falling` に戻ったことを検知 ← **採用**
3. **Promise方式**: 連鎖処理を Promise でラップ

**実装**: `clearErasingPuyos` 内で `advancePhase` 後に `phase` が `falling` または `gameover` に遷移したことで連鎖完了を検知。

### ゲームオーバー時の扱い

- 連鎖完了後にゲームオーバーになった場合もスナップショットを作成する
- 連鎖途中でゲームオーバーになることはない（連鎖中はぷよが消えていく方向なので）

### 連鎖途中での中断

- 連鎖途中で Back ボタンやアプリ終了した場合、その手のスナップショットは作成されない
- Resume 時は前の手の完了状態から再開される（その手は「なかったこと」になる）
- これにより Resume 時に不自然な状態にならない
